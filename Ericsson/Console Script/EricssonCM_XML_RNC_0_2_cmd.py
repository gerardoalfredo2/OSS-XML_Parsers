                        ...
                        for child5 in child4.findall('{utranNrm.xsd}RncFunction'):
                           ...
                           #This block parse the other MO in cell level
                                for child7 in child6.findall('{genericNrm.xsd}VsDataContainer'):
                                        ...
                                        for child9  in child8:
                                           ...
                                           # print 'child9: ',child9.tag,child9.text
                                            for child10  in child9:
                                                level = (
                                                str(child10.tag).replace('{EricssonSpecificAttributes.15.25.xsd}',
                                                                         '')).replace('{genericNrm.xsd}', '')
                                                if len(child10)>0:
                                                    temp2.clear()
                                                    for child11 in child10.iter():
                                                        headers.append(level + "." + (str(child11.tag).replace(
                                                            '{EricssonSpecificAttributes.15.25.xsd}', '')).replace(
                                                            '{genericNrm.xsd}', ''))
                                                        lines.append(str(child11.text).replace('\n', ''))
                                    ...
                                    temp=merge_dicts(temp1,temp)
                                    if os.path.exists(OutPut_dir+"CellLevel_"+str(temp.get('vsDataType'))+".txt")==False:
                                        logger('Date\t'+'RNC_name\t'+'\t'.join(temp1.keys()+headers).replace('\n',''),OutPut_dir+"CellLevel_"+str(temp.get('vsDataType'))+".txt")
                                   ...
                                   # print '\r'+str(dict(child6.attrib).get('id'))
                                    ...
                                    for child8 in child7:
                                        ...
                                        for child9 in child8:
                                            ...
                                            if len(child9)>0:
                                                level = (
                                                str(child9.tag).replace('{EricssonSpecificAttributes.15.25.xsd}',
                                                                        '')).replace('{genericNrm.xsd}', '')
                                                te={}
                                                for child11 in child9:
                                                    ...
                                                    str(child11.tag).replace('{EricssonSpecificAttributes.15.25.xsd}',
                                                                             '')).replace('{genericNrm.xsd}', ''))
                                                    lines.append(str(child11.text).replace('\n', ''))
